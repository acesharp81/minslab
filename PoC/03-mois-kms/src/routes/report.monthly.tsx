...
import { generateMonthlyDivisionReport } from "@/lib/report.functions";
import { ReportView } from "@/components/report/ReportView";
import { ModelSettingsPanel, useReportAISettings } from "@/components/report/ModelSettings";

export const Route = createFileRoute("/report/monthly")({
...
function MonthlyReportPage() {
  ...
  const mutation = useMutation({
    mutationFn: () => generateMonthlyDivisionReport({ data: { year, month, ...ai.settings } }),
  });
  ...
  const subtitle = mutation.data
    ? `${mutation.data.divisionName} · ${year}년 ${month}월 · 총 ${mutation.data.taskCount}건`
    : `${year}년 ${month}월`;
