...
AI 언론동향 비서는 기관 키워드로 뉴스를 공동 수집하고, Groq `llama-3.1-8b-instant`가 기사별 공통 분류·어조·요약을 한 번 확정한 뒤 OpenRouter의 `google/gemma-4-26b-a4b-it:free`가 기사 1건당 최대 10개 케이스를 한 번에 독립 판정하는 뉴스 모니터링 PoC입니다. 로컬 Ollama는 기사·케이스 프롬프트·보도자료 임베딩에만 사용합니다. 벡터 유사도 50%와 LLM 의미판정 50%, 근거 검증과 전송 기준을 모두 통과한 기사만 카카오톡 개인별 `나와의 채팅`에 제목, 요약, 원문 링크를 전송합니다.
...
기존 Nginx
  └─ 기존 Uvicorn main.py
       ...
       └─ 기존 ASGI lifespan
            ...
            ├─ Common Worker 1개: Groq 공통 분류·어조·요약
            ├─ Embedding Worker 1개: Ollama 기사·케이스·보도자료 임베딩
            ├─ Remote Worker 1개: OpenRouter Gemma 최대 10개 케이스 배치 판정
            ├─ Remote Burst Worker 1개: 배치 대기 10건 이상일 때 추가 처리
...
2. Local 공통 분석
   Groq가 기사별 핵심 문장을 한 번 읽고 주제 분류·배타적 어조·요약·엔터티·공통 근거를 확정
3. Local 임베딩·후보 선정
   기사 벡터와 버전별 케이스 프롬프트 벡터를 캐시하고, 키워드 또는 보정 벡터 유사도로 판정 후보를 선정
4. Remote 케이스 배치 판정
   같은 기사 후보를 최대 10개 묶어 OpenRouter Gemma 1회 호출로 전달하고, 케이스별 관련 여부·연속 점수·근거를 독립 반환
5. Hybrid 확정·발송
   보정 벡터 점수 50%와 LLM 점수 50%를 합산하고, 관련 여부·대상·주제·어조·제외어 하드 게이트를 통과한 건만 독립 발송
```

`주제 검색 사용자 프롬프트`는 케이스별 의미판정의 핵심 운영 설정이며 OpenRouter 배치 요청에 그대로 포함됩니다. 기사 원문 전체는 보내지 않고 로컬에서 선별한 공개 근거 문장만 전송합니다. 현재 PoC의 부정 모니터링은 `행정안전부 관련 부정적 기사`처럼 짧게 작성해도 공통 시스템 규칙이 기관 직접 비판과 운영 사실 예외를 적용합니다. 재난 케이스는 이름 또는 설정에 `재난`, `사건`, `사고`, `안전` 중 핵심 범위를 명시합니다.
...
6. Groq 공통분석은 기사를 한 번만 읽고, 독립 Ollama Worker가 기사·케이스 벡터를 캐시한 뒤, OpenRouter는 동일 기사 케이스를 최대 10개씩 묶어 최소 공개 증거로 판정합니다.
...
```text
실제 유사도 = 보정 벡터 유사도 × 0.5 + OpenRouter 의미판정 × 0.5
발송 조건 = 실제 유사도 ≥ 케이스 임계값 AND LLM 관련 판정 AND 로컬 대상·주제·어조·제외어 검증 통과
```

키워드 점수는 후보 선정과 분석보고서 진단값으로만 사용하며 표시 퍼센트에 섞지 않습니다. 기사 벡터는 기사마다 한 번, 케이스 프롬프트 벡터는 케이스 버전마다 한 번 생성해 재사용합니다. 벡터 코사인값은 해당 케이스의 기사 분포(q10·q50·q90)로 보정해 밀집된 원시값을 0~100으로 펼칩니다. OpenRouter에는 사용자가 입력한 케이스 프롬프트와 공개 기사에서 선별한 공유 근거를 보내되 원문 전체는 보내지 않습니다. 단순 언급·연관성 부족·다른 맥락·본문 부족은 대시보드에 `발송 제외`로 남고, LLM 장애나 배치 응답 일부 누락은 해당 케이스만 재시도하며 임의 발송하지 않습니다.
...
- `PUT /api/poc/master-press/admin/settings/common-llm-model`
- `PUT /api/poc/master-press/admin/settings/case-llm-model`
- `PUT /api/poc/master-press/admin/settings/embedding-model`
