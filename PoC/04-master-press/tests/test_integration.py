...
class MainIntegrationTests(unittest.TestCase):
    def test_static_dashboard_and_project_registration(self):
        ...
        self.assertEqual(body.count(b'<script src="/poc/master-press/magazine_reader.js?v='), 1)
        self.assertLess(body.index(b'magazine_reader.js'), body.index(b'app.js'))
        self.assertEqual(body.count(b'<script src="/poc/master-press/magazine_speech.js?v='), 1)
        self.assertLess(body.index(b'magazine_reader.js'), body.index(b'magazine_speech.js'))
        self.assertLess(body.index(b'magazine_speech.js'), body.index(b'app.js'))
        reader_status, _reader_headers, reader_body = asyncio.run(call_app("/poc/master-press/magazine_reader.js"))
        speech_status, _speech_headers, speech_body = asyncio.run(call_app("/poc/master-press/magazine_speech.js"))
        self.assertEqual(reader_status, 200)
        self.assertIn(b'buildNarration', reader_body)
        self.assertEqual(speech_status, 200)
        self.assertIn(b'createController', speech_body)
        app_status, _app_headers, app_body = asyncio.run(call_app("/poc/master-press/app.js"))
        self.assertEqual(app_status, 200)
        self.assertIn(b'stopMagazineReadForContext', app_body)
        self.assertIn(b"window.addEventListener('pagehide'", app_body)
        self.assertIn(b'updateMagazineReadIssueHighlight', app_body)
        self.assertIn(b'scrollMagazineReadIssueToTop', app_body)
        self.assertIn(b'magazineReadFloatingToggle', app_body)
        self.assertIn(b'acquireMagazineWakeLock', app_body)
        self.assertIn(b"navigator.wakeLock.request('screen')", app_body)
        self.assertIn(b"document.addEventListener('visibilitychange'", app_body)
        self.assertIn(b'topbar.getBoundingClientRect().bottom', app_body)
        ...
        self.assertIn(b'.magazine-issue{scroll-margin-top:82px}', styles_body)
        self.assertIn(b'prefers-reduced-motion:reduce', styles_body)
        ...
        self.assertIn('aria-label="매거진 읽어주기"'.encode("utf-8"), body)
        self.assertIn(b'id="magazineReadStart"', body)
        self.assertIn(b'id="magazineReadPause"', body)
        self.assertIn(b'id="magazineReadStop"', body)
        self.assertIn(b'id="magazineReadRate"', body)
        self.assertIn(b'<option value="1.25">x1.25</option>', body)
        self.assertIn(b'<option value="1.5" selected>x1.5</option>', body)
        self.assertIn(b'<option value="1.75">x1.75</option>', body)
        self.assertIn(b'id="magazineReadProgress"', body)
        self.assertIn(b'id="magazineReadStatus"', body)
        self.assertIn(b'id="magazineReadFloatingToggle"', body)
